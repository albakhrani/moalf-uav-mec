...
- **Update (eq 27):** `Q_rl(s,a) ← Q_rl(s,a) + α_lr·[ R_MORL + γ·max_{a'} Q_rl(s',a') − Q_rl(s,a) ]`.
- **Exploration:** ε-greedy, `ε: 1.0 → ×0.995/episode`.
- **Hyperparams:** `α_lr=0.001, γ=0.99, replay=10000, batch=32, n_ep` (§14).
- **Note on state/action tabularity:** Alg. 2 is tabular Q-learning, but the state is continuous/high-dimensional. A coder must define a discretization or function approximator. 🔲 **SIGN-OFF (tabular discretization vs function approximation).**
...
- **Role:** given MORL's assignment `x` and MPC's positions `p`, choose continuous `f_ijk ∈ [0,C_j]` to minimize the APSO projection `w1·J̃_task + w2·J̃_energy + w5·J̃_util` (§6), subject to capacity (22).
...
| `V` (Lyapunov) | 1.0 | neutral start; to be swept (§10.1) |
...
- [ ] §7 MORL state/action discretization vs function approximation — *still open (implementation-time choice)*
...
> One item remains genuinely open: §7 tabular-vs-approximator is an implementation-time decision, not a value. Per the author's order of operations, approved values are **not yet folded into `config/default.yaml`** and **no code is written** — both are held until the author reviews the rewritten §4.7 and §10.
