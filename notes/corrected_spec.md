...
44. **§18.4 REPRODUCTION RESULT — 2026-06-23 (frozen config, 30 runs seeds 42–71; nothing tuned):** results in `results/comparison_table.md`. **Completion 94.5% → reproduced 98.4%±0.5 (Tier 1, even exceeds).** **Throughput +55% → +0.6%±5.3 at frozen load (Tier 3 divergent)** because underloaded (random already completes 98%; entries 32/39) — but **+16.7%±6.9 at high load (Tier 2)**, so the throughput mechanism is real and only emerges under stress. **Route −38% → −11.9%±15.3 (Tier 3 divergent)**: coverage-driven MPC micro-adjusts rather than minimizing travel, and saturated SNR gives position weak leverage (entries 24/43) → routes not shortened (baseline is also a stand-in). **Honest assessment:** 1/3 headline metrics reproduces cleanly; the two relative-improvement claims do not reproduce at the stated parameters — *explained* by the lightly-loaded, saturated-SNR regime (the paper's numbers likely came from a more-stressed regime and/or its own baselines). Per §18: a Tier-3 result reported as a finding, no value tuned to close the gap. *(§18.4 / entries 24/32/39/43 / experiments/reproduce_table.py)*
